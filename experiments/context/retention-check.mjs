...
for (const bytes of [64 * 1024, 1024 * 1024, 4 * 1024 * 1024]) {
 const base = Array.from({length: 64}, (_,i) => ({role:i % 2 ? 'assistant' : 'user',content:'x'.repeat(bytes / 64)}));
 ...
 rows.push({historyBytes:bytes,serializationMedianMs:+baselineMs.toFixed(3),retentionMedianMs:+retentionMs.toFixed(3)});
}
